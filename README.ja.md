...
インデックス作成がディスクを静かに埋めたり、マシンを重くしたりしないよう、決められた予算の
なかで動きます。クラスタとトピックの数、トピックあたりのインデックス件数、インデックスが使える
ディスク、一度の検索が持てる複雑さに、それぞれ上限があります。

**現在適用されている値はアプリの設定画面に表示され**、アプリは表示どおりに動きます。
ビルドに固定された値ではなくサービスから受け取るため、更新をお願いすることなく調整されることが
あります。正確な答えはアプリ側にあり、ここに数字を書いても古くなるだけです。

| | |
|---|---|
| **ディスク** | クラスタごとに上限があり、より低く設定できます。必要な空き容量は[ダウンロード](#ダウンロード)を参照 |
| **規模** | 1 つのトピックにつき数百万件までインデックスします |
| **上限を超えたら** | インデックス作成は止まりません — そのトピックに選んだポリシーに従って、価値の低いインデックスから整理されます |
