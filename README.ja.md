...
Apple Silicon / Intel のうち、この Mac に合うビルドが自動的に選ばれます。初回起動時は、[macOS の初回のみの許可手順](#インストール)に従ってください。
...
| | |
|---|---|
| **サイズ** | ダウンロード 75〜85 MB、インストール後 約 105 MB |
...
この Mac に合うビルドをインストールし、初回起動も試みます。Homebrew から
`/Applications/Kaflow Search.app` がすでに存在すると表示された場合は、Kaflow Search を終了し、
以前手動でインストールしたアプリをアプリケーションフォルダの外へ移動してから、もう一度実行してください。
設定とローカルインデックスは `~/.kaflow` にあるため、アプリ自体を移動しても削除されません。

下の手順 2 の警告が表示されたら **OK** を押し、手順 3 に進んでください。
...
v0.1.3 以降は Microsoft Visual C++ ランタイムがアプリに組み込まれているため、別途インストールする
必要はありません。まだ v0.1.2 Windows Preview を使用していて起動直後に終了する場合は、最新リリースへ
更新してください。その Preview ビルドを使い続ける必要がある場合は、
[Microsoft Visual C++ 再頒布可能パッケージ (x64)](https://aka.ms/vs/17/release/vc_redist.x64.exe)
をインストールできます。旧バージョンのランタイム問題は、Windows イベント ビューアーに
`MSVCP140.dll`、`VCRUNTIME140.dll`、または例外コード `0xc0000005` と表示されることがあります。

それでも起動しない場合は、Windows のバージョンと Kaflow Search に関するイベント ビューアーの
エラーを添えて[バグを報告してください](https://github.com/whsoul/kaflow-search/issues/new?template=bug_report.yml)。
...
Kafka クラスタが大きくても、クラスタ全体を手元のコンピュータへ複製するわけではありません。
必要なトピックだけを選び、設定したストレージ予算の範囲内でローカル検索インデックスを管理します。
...
| **選択的なインデックス作成** | 必要なトピックだけを選択します。クラスタ内のすべてのトピックを自動でインデックス化することはありません |
| **上限のあるローカルストレージ** | 設定可能なストレージ予算とクリーンアップポリシーにより、ローカルインデックスの無制限な増加を防ぎます |
...
| **Kaflow に送られるもの** | 起動時と、アプリを開いている間は数時間ごとに、バージョンサポートとお知らせのために: ランダムなインスタンス ID · ハッシュ化したマシン ID · アプリのバージョン · OS / アーキテクチャ · UI の言語。Kafka の構成やメッセージについては何も送りません |
...
**バージョン 0.1.3 — オープンベータです。** 日常の作業に使えます。
...
| | |
|---|---|
| **ソース公開型で、オープンソースではありません** | フロントエンドのソースと検索エンジンの実装はここに**ありません** |
