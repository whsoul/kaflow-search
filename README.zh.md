...
**[⬇ 下载](#下载)** · **[安装指南](#安装)** · [Issues](https://github.com/whsoul/kaflow-search/issues)
...
>
> <sub>如果[赞助](#支持本项目)能够积累起来，开发者认证会是第一笔支出。</sub>
...
| **发送给 Kaflow 的内容** | 仅在启动时，用于版本支持与公告：随机实例 ID · 哈希后的机器 ID · 应用版本 · 操作系统/架构 · 界面语言。不包含任何关于你的 Kafka 配置或消息的信息 |
...
建索引不应悄悄占满磁盘或拖慢机器，因此应用始终在既定预算内工作。集群和 topic 的数量、单个
topic 的索引条数、索引可用的磁盘、以及一次检索能有多复杂，都各有上限。

**当前生效的数值会显示在应用的设置界面里**，应用严格按显示的值执行。它们不是在构建时写死的，
而是从服务端获取，因此无需你更新也可能调整。准确的答案在应用里，写在这里的数字只会过时。

| | |
|---|---|
| **磁盘** | 按集群设上限，你也可以调得更低。所需空余空间见[下载](#下载) |
| **规模** | 单个 topic 可索引至数百万条 |
| **超过上限后** | 建索引不会停止 —— 会按你为该 topic 选择的策略，优先清理价值较低的索引 |
...
**优先用在哪里：** macOS 和 Windows 的代码签名证书，好让[安装时的警告](#安装)消失。
之后是在更多平台和 Kafka 版本上做验证、发布基础设施，以及文档。
