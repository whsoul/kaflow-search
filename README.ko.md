...
인덱싱이 디스크를 소리 없이 채우거나 컴퓨터를 무겁게 만들지 않도록, 정해진 예산 안에서 동작합니다.
클러스터·토픽 개수, 토픽당 인덱스 건수, 인덱스가 쓸 수 있는 디스크, 한 번의 검색이 가질 수 있는
복잡도에 각각 상한이 있습니다.

**지금 적용되는 값은 앱의 설정 화면에 그대로 나오며**, 앱은 거기 표시된 값을 그대로 지킵니다.
빌드에 고정된 값이 아니라 서비스에서 받아오기 때문에, 업데이트 없이도 조정될 수 있습니다.
그래서 정확한 답은 앱이고, 여기에 숫자를 적어두면 낡기만 합니다.

| | |
|---|---|
| **디스크** | 클러스터별로 상한이 있고, 더 낮게 잡을 수 있습니다. 필요한 여유 공간은 [다운로드](#다운로드) 참고 |
| **규모** | 토픽 하나에 수백만 건까지 인덱싱합니다 |
| **한도를 넘으면** | 인덱싱이 멈추지 않습니다 — 토픽에 지정한 정책에 따라 가치가 낮은 인덱스부터 정리됩니다 |
