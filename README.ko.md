...
`로컬 인덱싱` · `별도 인프라 불필요` · `가입 없음` · `데이터는 내 PC에`
...
**안전합니다** — 클라우드가 아니라 **내 PC에 설치** · 메시지가 PC를 벗어나지 않음 · 비밀번호 미저장
...
| | |
|---|---|
| **메시지·인덱스·검색어** | 내 PC에서만 처리. Kaflow 서버로 전송되지 않음 |
...
로컬 PC의 안정성을 해치지 않는 선에서 정한 값이며, 앞으로 조정될 수 있습니다.
