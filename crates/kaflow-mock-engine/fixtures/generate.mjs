// mock-engine 데모 fixture 생성기 (deterministic — seed 고정이라 재실행해도 동일 출력).
//
//   node crates/kaflow-mock-engine/fixtures/generate.mjs
//
// 고카디널리티 + 다양한 필드의 합성 데이터를 만들어 default.json 으로 덮어쓴다.
// offset 은 생략(로더가 partition 별 자동증가), ts 는 partition 내 offset 과 단조증가하게 배치.
...
function tsFor(i, n) {
  // partition 무관 전역 단조 + jitter — partition 별 offset 과도 단조 유지.
...
const root = {
  _comment:
    "GENERATED by generate.mjs (seed 20260626) — 수정하려면 스크립트를 고치고 재생성하세요. 고카디널리티 + 다국어 데모 데이터.",
  topics: [
    buildOrders(420, 4),
    buildSignups(300, 3),
    buildLogs(480, 4),
